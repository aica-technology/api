...
The System application graph is a unique way of expressing complex and reactive logic for robotic control. It combines
...
In graph terminology, a node is a block in the graph and an edge is a connecting line. The nodes in the System application
...
are followed by a list of all available components from Core and any additionally installed packages, grouped by
...
<CenteredImage src={EventEdge} alt={"aica-studio-event-edge.png"} />
...
<CenteredImage src={EditingGraphEdges} alt={"aica-studio-editing-graph-edges"} />
