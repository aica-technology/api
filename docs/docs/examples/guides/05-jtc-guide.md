...
First, start AICA Launcher and create a configuration with at least `AICA Core v4.4.0 ???` (TODO: joint positions). For the remainder of this guide,
...
- `times_from_start`: the time from the start of the trajectory that each waypoint should be reached (in seconds)
...
To do so, we need a sequence block that:
