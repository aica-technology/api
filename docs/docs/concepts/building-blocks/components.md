...
Components define signals as inputs (subscribers) and outputs (publishers). Refer to [Signals](./signals) for more
information.
...
to trigger events. Refer to [Events](./events) for more information.
...
In the System, lifecycle components can auto-configure and auto-activate themselves
using [transition events](./events#transitions). Associating the `on_load` state transition with a
